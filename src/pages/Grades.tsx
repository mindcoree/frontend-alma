          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold">Current GPA</h3>
                <AcademicCapIcon className="w-6 h-6" />
              </div>
              <div className="flex items-baseline space-x-2">
                <span className="text-3xl font-bold">{calculateGPA(gradesInSemester)}</span>
                <span className="text-sm text-white/70">/ 4.00</span>
                {gpaTrend && (
                  <span className={`text-sm ${gpaTrend.color}`}>
                    {gpaTrend.text}
                  </span>
                )}
              </div>
